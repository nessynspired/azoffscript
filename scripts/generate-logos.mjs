...
function stackedSVG({ textColor, bgColor, includeBg }) {
  const W = 800;
  const H = 800;
  const fontStack = "'Arial Black', 'Helvetica Neue', Helvetica, Arial, sans-serif";
  const bgRect = includeBg
  ...
  <g font-family="${fontStack}" font-weight="900" text-anchor="middle">
    <text x="${W / 2}" y="${H / 2 - 20}" font-size="200" fill="#f5b800" letter-spacing="-6">AZ</text>
...
(async () => {
  ...
  await render("logo-white-on-black.png", wordmarkSVG({ textColor: "#ffffff", bgColor: "#000000", includeBg: true }));
  await render("logo-black-on-white.png", wordmarkSVG({ textColor: "#1a1a1a", bgColor: "#ffffff", includeBg: true }));

  // Stacked (square) lockups — good for avatars / profile pics
  ...
  await render("logo-stacked-white-on-black.png", stackedSVG({ textColor: "#ffffff", bgColor: "#000000", includeBg: true }));
  await render("logo-stacked-black-on-white.png", stackedSVG({ textColor: "#1a1a1a", bgColor: "#ffffff", includeBg: true }));
