/**
 * Quick Drop Library — AZ Off Script content format library.
 *
 * POSITIONING: AZ Off Script is an Arizona creator brand. The crew
 * is the first women-led room. Not about one type of person — about what
 * happens when different Arizona personalities get the same prompt and
 * nobody answers the same way.
 *
 * PHILOSOPHY: Prompt them. Don't script them.
 * We give the idea. They bring the moment.
 *
 * Arizona is the setting. Social scripts are the engine.
 *
 * Each template has:
 *  - Idea, Vibe, WhatToDrop, EasyWay, Examples, MakeItYours
 *  - Effort label: "2-Min Drop", "5-Min Drop", "10-Min Drop", "Group Day", "Edit Heavy"
 *  - Home-friendly, needs talking, needs editing flags
 *  - SEO phrase, caption starter, hashtag starter (5-8, not 25)
 *  - Category bucket (format type)
 *  - Topic world (subject lane: Arizona Life, Dating, Friendship, etc.)
 *  - Clear instructions: whatThisIs, whatEachPersonDoes, whatToSend,
 *    examplePrompts, whyItWorks, bestFor
 */

export type EffortLabel = "2-Min Drop" | "5-Min Drop" | "10-Min Drop" | "Group Day" | "Edit Heavy" | "Save Later";

export type TopicWorld =
  | "Arizona Life"
  | "Men, Women & the Script"
  | "Dating"
  | "Texting"
  | "Friendship"
  | "Mom-Life"
  | "Family"
  | "Adulting"
  | "Beauty / Style"
  | "Soft Truths"
  | "Public Behavior"
  | "Work / Money"
  | "Group Chat"
  | "Brand Collabs"
  | "Crew / Future Wave";

export interface QuickDropTemplate {
  id: string;
  name: string;
  bucket: string;
  topicWorld: TopicWorld;
  description: string;
  effort: EffortLabel;
  timeEstimate: string;
  homeFriendly: boolean;
  needsTalking: boolean;
  needsEditing: boolean;
  adminStitches: boolean;
  maxSeconds?: number;
  idea: string;
  vibe: string;
  whatToDrop: string;
  easyWay?: string;
  examples?: string[];
  makeItYours: string;
  transitions?: string[];
  seoPhrase: string;
  captionStarter: string;
  hashtagStarter: string[];
  platforms: string[];
  // Clear instructions so crew knows exactly what to do
  whatThisIs?: string;
  whatEachPersonDoes?: string;
  whatToSend?: string;
  examplePrompts?: string[];
  whyItWorks?: string;
  bestFor?: string;
  // Optional: links this template to a prebuilt Shot Recipe from the library.
  // When a clip is created from this template, the recipe auto-attaches so
  // crew members see "Your Assignment" steps without a planner having to
  // open Build Recipe → Pick from Bank manually.
  shotRecipeId?: string;
}

export const CONTENT_BUCKETS = [
  "Transitions",
  "Face-Only Reactions",
  "One-Line Verdicts",
  "Arizona Moments",
  "Group Chat Court",
  "Soft POVs",
  "BTS / Real Process",
  "AZ Tried It",
  "Friendship Energy",
  "Comment-to-Clip",
  "Search Explainers",
  "Arizona Real Life",
  "What Type Are You",
  "Quiet Girl Friendly",
  "Off Script Looks",
  "Real But Not Too Deep",
  "Fake Scenario Court",
  "Arizona Woman Math",
  "Text Translation",
  "Rank It",
  "Room Temperature Check",
  "Don't Overthink It",
  "Two-Second Opinion",
  "Different Women Different Answer",
  "Crew / Future Wave",
  "Black + Latina Lens",
  // New format buckets
  "Men, Women & the Script",
  "Dating Scripts",
  "Texting Scripts",
  "Friendship Court",
  "Mom-Life Off Script",
  "Family Scripts",
  "Adulting & Errands",
  "Beauty / Style / Presentation",
  "Soft Truths",
  "Public Behavior",
] as const;

export const TOPIC_WORLDS: TopicWorld[] = [
  "Arizona Life",
  "Men, Women & the Script",
  "Dating",
  "Texting",
  "Friendship",
  "Mom-Life",
  "Family",
  "Adulting",
  "Beauty / Style",
  "Soft Truths",
  "Public Behavior",
  "Work / Money",
  "Group Chat",
  "Brand Collabs",
  "Crew / Future Wave",
];

export const QUICK_DROP_TEMPLATES: QuickDropTemplate[] = [
  // ===== TRANSITIONS =====
  {
    id: "first_wave_intro",
    name: "Crew Intro Pass",
    bucket: "Transitions",
    topicWorld: "Arizona Life",
    description: "Each person records a short intro using a simple camera transition. Stitched together into one intro video.",
    effort: "10-Min Drop",
    timeEstimate: "5-10 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: true,
    adminStitches: true,
    maxSeconds: 10,
    idea: "Introduce yourself as part of AZ Off Script using a simple camera transition.",
    vibe: "Confident, funny, chill, real. Don't overthink it.",
    whatToDrop: "A short intro clip, 5-10 seconds.",
    easyWay: "Step toward the camera, cover the camera, zoom in/out, or do your own transition.",
    examples: [
      "I'm Vanessa, and I'm building the room.",
      "I'm Ronnie, and I bring the sweet touch.",
      "I'm Sholanda, and somebody had to say it.",
      "I'm Elaine, and my face says enough.",
      "I'm Latasha, and I'm the wild card.",
      "I'm Maria, and I bring the fresh energy.",
    ],
    makeItYours: "Say your name, your vibe, or why people need to watch you. Add your own attitude, humor, face, pose, line, or little moment. This is not a script — bring your own timing, face, attitude, or twist.",
    transitions: [
      "Step toward the camera",
      "Cover the camera with your hand",
      "Cover the camera with a tumbler or phone",
      "Zoom into your face",
      "Start close-up, then step back",
      "Walk into frame",
      "Point at the camera",
      "Do your own transition",
    ],
    seoPhrase: "Arizona creator crew",
    captionStarter: "Meet the AZ Off Script crew. Who's your favorite?",
    hashtagStarter: ["#AZOffScript", "#ArizonaCreators", "#ArizonaTikTok", "#CreatorCrew", "#MeetTheCrew"],
    platforms: ["TikTok", "Reels", "Facebook", "Shorts"],
    shotRecipeId: "recipe_crew_intro_b",
  },
  {
    id: "step_into_the_room",
    name: "Step Into the Room",
    bucket: "Transitions",
    topicWorld: "Arizona Life",
    description: "Each person steps toward the camera and gives one line.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: true,
    adminStitches: true,
    maxSeconds: 8,
    idea: "Step toward the camera and give one line — your answer to the prompt.",
    vibe: "Quick, confident, your entrance is the energy.",
    whatToDrop: "A short clip stepping into frame with your answer. 5-8 seconds.",
    easyWay: "Start off-frame, step in, say your line, step out.",
    examples: [
      "My answer is yes, and I'm not explaining.",
      "That's a no from me.",
      "I'm not saying it's me, but it's me.",
    ],
    makeItYours: "Your walk, your line, your energy. The step is the style — your personality is the content.",
    transitions: ["Step toward camera", "Walk into frame", "Doorway intro", "Do your own"],
    seoPhrase: "transition reaction video",
    captionStarter: "We stepped into the room with our answers. What's yours?",
    hashtagStarter: ["#AZOffScript", "#ArizonaCreators", "#TransitionTrend", "#ReactionVideo", "#StepIntoTheRoom"],
    platforms: ["TikTok", "Reels", "Shorts"],
  },
  {
    id: "zoom_verdict",
    name: "Zoom Verdict",
    bucket: "Transitions",
    topicWorld: "Arizona Life",
    description: "Start close, zoom out, answer the question.",
    effort: "2-Min Drop",
    timeEstimate: "1-2 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 10,
    idea: "Start close-up, zoom out, and give your verdict on the prompt.",
    vibe: "Dramatic, quick, satisfying zoom reveal.",
    whatToDrop: "A zoom-out clip with your answer. 5-10 seconds.",
    easyWay: "Start with camera close to your face, zoom out as you answer.",
    makeItYours: "Your zoom speed, your face, your answer. Make the reveal yours.",
    transitions: ["Zoom out from face", "Zoom into face", "Do your own"],
    seoPhrase: "zoom reaction video",
    captionStarter: "Zoom out for the verdict. Did we get it right?",
    hashtagStarter: ["#AZOffScript", "#ZoomVerdict", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts"],
  },
  {
    id: "cover_camera",
    name: "Cover the Camera",
    bucket: "Transitions",
    topicWorld: "Arizona Life",
    description: "Use hand, tumbler, purse, phone, shirt, or sunglasses to cover and reveal.",
    effort: "5-Min Drop",
    timeEstimate: "2-5 min",
    homeFriendly: true,
    needsTalking: false,
    needsEditing: true,
    adminStitches: true,
    maxSeconds: 10,
    idea: "Cover the camera with something, then reveal your answer or reaction.",
    vibe: "Smooth, satisfying, creative. The cover is the style.",
    whatToDrop: "A cover-and-reveal clip. 5-10 seconds.",
    easyWay: "Cover the lens with your hand or an object, pull away to show your answer.",
    makeItYours: "Use whatever's near you — tumbler, phone, sunglasses, purse. Your object, your reveal.",
    transitions: ["Cover with hand", "Cover with tumbler", "Cover with phone", "Cover with sunglasses", "Cover with shirt", "Do your own"],
    seoPhrase: "cover camera transition",
    captionStarter: "Cover, reveal, react. How would you answer?",
    hashtagStarter: ["#AZOffScript", "#CoverCamera", "#TransitionTrend", "#ArizonaCreators"],
    platforms: ["TikTok", "Reels", "Shorts"],
  },
  {
    id: "walk_off_answer",
    name: "Walk-Off Answer",
    bucket: "Transitions",
    topicWorld: "Arizona Life",
    description: "Answer the prompt and walk out of frame.",
    effort: "2-Min Drop",
    timeEstimate: "1-2 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 8,
    idea: "Answer the prompt, then walk out of frame like you're done.",
    vibe: "Confident, final, a little sassy. Said it, done.",
    whatToDrop: "Your answer + walk off. 5-8 seconds.",
    easyWay: "Hit record, say your answer, walk away.",
    makeItYours: "Your walk, your energy. The walk-off is the mic drop.",
    transitions: ["Walk out of frame", "Turn and leave", "Do your own"],
    seoPhrase: "walk off reaction video",
    captionStarter: "Said it and walked off. Would you do the same?",
    hashtagStarter: ["#AZOffScript", "#WalkOff", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts"],
  },
  {
    id: "object_pass",
    name: "Object Pass",
    bucket: "Transitions",
    topicWorld: "Arizona Life",
    description: "Pass the tumbler, phone, sunglasses, or cactus sticker to the next person.",
    effort: "Group Day",
    timeEstimate: "10-15 min",
    homeFriendly: true,
    needsTalking: false,
    needsEditing: true,
    adminStitches: true,
    maxSeconds: 60,
    idea: "Pass an object to the camera to transition to the next person's clip.",
    vibe: "Connected, smooth, group energy. The object is the link.",
    whatToDrop: "Your clip — end by passing an object toward the camera.",
    easyWay: "Record your part, then toss or hand an object toward the lens.",
    makeItYours: "Your object, your throw, your energy. The pass connects the room.",
    transitions: ["Pass tumbler", "Pass phone", "Pass sunglasses", "Pass cactus sticker", "Do your own"],
    seoPhrase: "pass the object transition",
    captionStarter: "We passed it around the room. Who had the best answer?",
    hashtagStarter: ["#AZOffScript", "#ObjectPass", "#ArizonaCreators", "#GroupVideo", "#TransitionTrend"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
  },
  {
    id: "chair_spin",
    name: "Chair Spin",
    bucket: "Transitions",
    topicWorld: "Arizona Life",
    description: "Start turned away, spin to camera, answer.",
    effort: "2-Min Drop",
    timeEstimate: "1-2 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 8,
    idea: "Start turned away from the camera, spin around, and give your answer.",
    vibe: "Playful, dramatic, a little theatrical. The spin is the reveal.",
    whatToDrop: "A chair-spin clip with your answer. 5-8 seconds.",
    easyWay: "Sit turned away, spin to face the camera, say your line.",
    makeItYours: "Your spin speed, your face when you land, your answer. Make the spin yours.",
    transitions: ["Chair spin to camera", "Turn head to camera", "Do your own"],
    seoPhrase: "chair spin reaction",
    captionStarter: "Spun around with the answer. What would you say?",
    hashtagStarter: ["#AZOffScript", "#ChairSpin", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts"],
  },

  // ===== FACE-ONLY REACTIONS =====
  {
    id: "face_only_reaction",
    name: "Face-Only Reaction",
    bucket: "Face-Only Reactions",
    topicWorld: "Arizona Life",
    description: "No talking — just your face reacting to a prompt.",
    effort: "2-Min Drop",
    timeEstimate: "1-2 min",
    homeFriendly: true,
    needsTalking: false,
    needsEditing: false,
    adminStitches: false,
    maxSeconds: 10,
    idea: "React to a prompt with your face only — no talking.",
    vibe: "Raw, real, unfiltered. Your face does all the work.",
    whatToDrop: "Your reaction face. That's it. No words needed.",
    easyWay: "Read the prompt, hit record, give your honest face.",
    examples: [
      "Side-eye",
      "Slow blink",
      "Walk away",
      "Fake smile",
      "Stare into camera",
      "Deep sigh",
    ],
    makeItYours: "There's no script here. Your face is the content. Whatever you feel, show it.",
    seoPhrase: "face reaction video",
    captionStarter: "No words needed. Just the face. What's yours?",
    hashtagStarter: ["#AZOffScript", "#FaceReaction", "#ReactionVideo", "#ArizonaCreators", "#NoWords"],
    platforms: ["TikTok", "Reels", "Shorts"],
  },

  // ===== ONE-LINE VERDICTS =====
  {
    id: "one_line_verdict",
    name: "One-Line Verdict",
    bucket: "One-Line Verdicts",
    topicWorld: "Arizona Life",
    description: "One sentence, strong opinion. No long explanation.",
    effort: "2-Min Drop",
    timeEstimate: "1-2 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: false,
    maxSeconds: 10,
    idea: "Give your verdict in one sentence. No hedging, no explaining.",
    vibe: "Bold, quick, final. Say it and done.",
    whatToDrop: "One line. Your verdict. 5-10 seconds.",
    easyWay: "Just hit record and say the first thing that comes to mind.",
    examples: [
      "Red flag. Next.",
      "Wrong, but I understand.",
      "That's not toxic, that's tired.",
      "Arizona made me this way.",
      "I'm not judging. I'm observing.",
      "Guilty. No further questions.",
    ],
    makeItYours: "Your words, your delivery. The shorter the better. Don't explain — just verdict.",
    seoPhrase: "hot take reaction",
    captionStarter: "One line. That's it. Do you agree?",
    hashtagStarter: ["#AZOffScript", "#HotTakes", "#OneLineVerdict", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
  },

  // ===== ARIZONA MOMENTS =====
  {
    id: "arizona_micro_moment",
    name: "Arizona Micro-Moment",
    bucket: "Arizona Moments",
    topicWorld: "Arizona Life",
    description: "Quick local take about living in Arizona.",
    effort: "5-Min Drop",
    timeEstimate: "2-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: false,
    maxSeconds: 20,
    idea: "Give us a quick Arizona take — something only AZ people get.",
    vibe: "Local, real, a little chaotic. Arizona-specific.",
    whatToDrop: "Your AZ take. 10-20 seconds.",
    easyWay: "Just hit record and say the first thing that comes to mind about Arizona life.",
    examples: [
      "Only Arizona people understand...",
      "Arizona hot take...",
      "West Valley behavior is...",
      "A 20-minute drive in Arizona feels like...",
      "The way we plan errands in July should be studied.",
      "Arizona people when it drops below 70.",
    ],
    makeItYours: "Your Arizona, your words. The more specific the better. West Valley, Buckeye, Phoenix — rep your area.",
    seoPhrase: "Arizona hot take",
    captionStarter: "Only Arizona people will understand this. You get it?",
    hashtagStarter: ["#AZOffScript", "#ArizonaTikTok", "#ArizonaHotTake", "#WestValleyAZ", "#ArizonaCreators", "#ArizonaLife"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
  },
  {
    id: "arizona_made_me",
    name: "Arizona Made Me This Way",
    bucket: "Arizona Moments",
    topicWorld: "Arizona Life",
    description: "How living in Arizona shaped your habits and instincts.",
    effort: "5-Min Drop",
    timeEstimate: "2-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: false,
    maxSeconds: 20,
    idea: "Finish the sentence: 'Arizona made me...' with a habit or instinct AZ gave you.",
    vibe: "Local, funny, real. Everyone in AZ has one.",
    whatToDrop: "Your 'Arizona made me' line. 10-20 seconds.",
    easyWay: "Just say what Arizona made you do or think.",
    examples: [
      "Arizona made me judge parking spots by shade first.",
      "Arizona made me think a 20-minute drive is a relationship test.",
      "Arizona made me plan errands like a survival mission.",
      "Arizona made me suspicious of anyone who says 'let's meet outside.'",
      "Arizona made me believe summer needs a cancellation policy.",
    ],
    makeItYours: "Your habit, your words. The more specific the funnier. Works with every personality.",
    seoPhrase: "Arizona made me this way",
    captionStarter: "Arizona made us this way. What did it do to you?",
    hashtagStarter: ["#AZOffScript", "#ArizonaMadeMe", "#ArizonaTikTok", "#ArizonaCreators", "#ArizonaLife"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    shotRecipeId: "recipe_arizona_made_me_b",
  },
  {
    id: "only_arizona_understand",
    name: "Only Arizona People Understand",
    bucket: "Arizona Moments",
    topicWorld: "Arizona Life",
    description: "Things only Arizona locals get.",
    effort: "5-Min Drop",
    timeEstimate: "2-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: false,
    maxSeconds: 15,
    idea: "Finish the sentence: 'Only Arizona people understand why...' with something local.",
    vibe: "Local, relatable, searchable. The 'you get it' energy.",
    whatToDrop: "Your 'Only Arizona' line. 10-15 seconds.",
    easyWay: "Just say something only AZ people would understand.",
    examples: [
      "Only Arizona people understand why 7 AM can already feel late.",
      "Only Arizona people understand why the steering wheel is personal.",
      "Only Arizona people understand why shade is a luxury item.",
      "Only Arizona people understand why 'just down the road' can mean 35 minutes.",
      "Only Arizona people understand why fall is a personality shift.",
    ],
    makeItYours: "Your Arizona moment. The more specific the better. Very searchable.",
    seoPhrase: "only Arizona people understand",
    captionStarter: "Only Arizona people will get this. Are you from here?",
    hashtagStarter: ["#AZOffScript", "#OnlyArizona", "#ArizonaTikTok", "#ArizonaCreators", "#ArizonaLife"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
  },

  // ===== GROUP CHAT COURT =====
  {
    id: "group_chat_court",
    name: "Group Chat Court",
    bucket: "Group Chat Court",
    topicWorld: "Group Chat",
    description: "The room decides — guilty or not guilty? Right or wrong?",
    effort: "10-Min Drop",
    timeEstimate: "5-10 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: true,
    adminStitches: true,
    maxSeconds: 20,
    idea: "We give you a scenario. You decide: guilty or not guilty? Right or wrong?",
    vibe: "Debate energy. Honest, funny, a little messy. The room decides.",
    whatToDrop: "Your verdict + a quick reason. 15-20 seconds.",
    easyWay: "Just say what you think and why. Don't overthink it.",
    examples: [
      "She invited everyone except one person because the vibe would be better. Wrong or fair?",
      "He watched every story but didn't text back. Guilty or not guilty?",
      "Your friend keeps showing up late. Still invite her or stop telling her the real time?",
    ],
    makeItYours: "Your verdict, your reasoning. Call it like you see it. The comments will vote too.",
    seoPhrase: "group reaction video",
    captionStarter: "The room decides: guilty or not guilty? You vote too.",
    hashtagStarter: ["#AZOffScript", "#GroupChatCourt", "#GroupReaction", "#ArizonaCreators", "#ReactionVideo", "#GuiltyOrNot"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
  },

  // ===== SOFT POVs =====
  {
    id: "soft_pov",
    name: "Soft POV",
    bucket: "Soft POVs",
    topicWorld: "Arizona Life",
    description: "Quick POV scenario — not full acting, just the moment.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: false,
    needsEditing: false,
    adminStitches: false,
    maxSeconds: 15,
    idea: "Act out a quick POV scenario. Just the moment, not a whole scene.",
    vibe: "Relatable, funny, recognizable. The 'oh that's me' energy.",
    whatToDrop: "A short POV clip. 10-15 seconds.",
    easyWay: "Just set up the scenario with on-screen text and react.",
    examples: [
      "POV: you said you were leaving but the car is 140 degrees.",
      "POV: the group chat says 'be honest.'",
      "POV: you're trying to pick a brunch spot with six women.",
      "POV: someone says 'I don't care' and definitely cares.",
    ],
    makeItYours: "Your face, your body language, your timing. The POV is the setup — your reaction is the content.",
    seoPhrase: "POV relatable video",
    captionStarter: "POV: you've been here. Tag someone who does this.",
    hashtagStarter: ["#AZOffScript", "#POV", "#Relatable", "#ArizonaCreators", "#POVTikTok"],
    platforms: ["TikTok", "Reels", "Shorts"],
  },

  // ===== BTS / REAL PROCESS =====
  {
    id: "bts_real_process",
    name: "BTS / Real Process",
    bucket: "BTS / Real Process",
    topicWorld: "Crew / Future Wave",
    description: "Behind the scenes — what it actually looks like to make content.",
    effort: "10-Min Drop",
    timeEstimate: "5-10 min",
    homeFriendly: true,
    needsTalking: false,
    needsEditing: false,
    adminStitches: false,
    maxSeconds: 30,
    idea: "Show what really happens when we try to make content. The chaos, the debates, the real.",
    vibe: "Unfiltered, honest, human. Not polished — real.",
    whatToDrop: "A BTS clip — the mess, the laughs, the process.",
    easyWay: "Just record what's happening. Don't perform — just document.",
    examples: [
      "Getting ready to film but the house is doing the most.",
      "What I thought content day would look like vs what actually happened.",
      "Behind the scenes of trying to make one simple clip.",
      "The 10-minute clip took 2 minutes and 8 mental debates.",
    ],
    makeItYours: "Show your real process. The mess is the content. Don't clean up — that's the point.",
    seoPhrase: "behind the scenes content creator",
    captionStarter: "This is what it actually looks like. No filter.",
    hashtagStarter: ["#AZOffScript", "#BTS", "#BehindTheScenes", "#ArizonaCreators", "#ContentCreator", "#RealProcess"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
  },

  // ===== AZ TRIED IT =====
  {
    id: "az_tried_it",
    name: "AZ Tried It",
    bucket: "AZ Tried It",
    topicWorld: "Arizona Life",
    description: "Try a local product, spot, or thing. Rate it.",
    effort: "Group Day",
    timeEstimate: "10-30 min",
    homeFriendly: false,
    needsTalking: true,
    needsEditing: true,
    adminStitches: true,
    maxSeconds: 60,
    idea: "Try something local — a drink, a spot, a product. Give your honest rating.",
    vibe: "Curious, honest, fun. Would we drive across town for this?",
    whatToDrop: "Your reaction to trying it + your rating.",
    easyWay: "Just record yourself trying it and react in real time.",
    examples: [
      "Rate this from 'never again' to 'put it in the rotation.'",
      "Would we drive across town for this?",
      "AZ Tried It: local drink edition.",
      "The room reacts to a local business.",
    ],
    makeItYours: "Your honest reaction. If you love it, say it. If you don't, say that too. Real reviews only.",
    seoPhrase: "Arizona local business reaction",
    captionStarter: "We tried it so you don't have to. Would you go?",
    hashtagStarter: ["#AZOffScript", "#AZTriedIt", "#ArizonaLocal", "#ArizonaCreators", "#LocalBusiness", "#ArizonaFood", "#WestValleyAZ"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
  },

  // ===== FRIENDSHIP ENERGY =====
  {
    id: "friendship_energy",
    name: "Friendship Energy",
    bucket: "Friendship Energy",
    topicWorld: "Friendship",
    description: "Warm crew content — the friend every group needs.",
    effort: "5-Min Drop",
    timeEstimate: "2-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 20,
    idea: "Describe the friend every group needs — or call out someone in the crew.",
    vibe: "Warm, funny, affectionate. The love is real.",
    whatToDrop: "Your friendship take. 15-20 seconds.",
    easyWay: "Just talk about your friends like you're in the group chat.",
    examples: [
      "The friend every group needs.",
      "The friend who says 'be safe' but also wants the tea.",
      "The friend who shows up late but brings snacks.",
      "The friend who can read your face across the room.",
    ],
    makeItYours: "Call out someone in the crew, or describe your friend type. The warmth is the content.",
    seoPhrase: "friendship group video",
    captionStarter: "Every group has this friend. Tag yours.",
    hashtagStarter: ["#AZOffScript", "#FriendshipEnergy", "#ArizonaCreators", "#GroupChemistry", "#FriendshipGoals"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
  },

  // ===== COMMENT-TO-CLIP =====
  {
    id: "comment_to_clip",
    name: "Comment-to-Clip",
    bucket: "Comment-to-Clip",
    topicWorld: "Group Chat",
    description: "Turn comments into content — respond to what viewers said.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 20,
    idea: "Read a comment from a previous post and respond to it.",
    vibe: "Responsive, engaged, a little spicy. The comments are writers too.",
    whatToDrop: "Your response to a viewer comment. 15-20 seconds.",
    easyWay: "Screen-record the comment, then record your response.",
    examples: [
      "Someone said we were wrong, so the room is responding.",
      "Comment court: this person said it's not a red flag.",
      "The comments were divided, so we're voting again.",
    ],
    makeItYours: "Your response, your energy. The comment is the prompt — your reaction is the content.",
    seoPhrase: "comment reaction video",
    captionStarter: "You said it, we're responding. What now?",
    hashtagStarter: ["#AZOffScript", "#CommentReaction", "#ArizonaCreators", "#ReactionVideo", "#CommentCourt"],
    platforms: ["TikTok", "Reels", "Shorts"],
  },

  // ===== SEARCH EXPLAINERS =====
  {
    id: "search_explainer",
    name: "Search Explainer",
    bucket: "Search Explainers",
    topicWorld: "Crew / Future Wave",
    description: "Explain what AZ Off Script is — for search and new viewers.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: false,
    maxSeconds: 15,
    idea: "Explain what AZ Off Script is in 10 seconds. For search, new viewers, and the algorithm.",
    vibe: "Clear, quick, confident. This is the elevator pitch.",
    whatToDrop: "A 10-second explainer. What is AZ Off Script?",
    easyWay: "Just say what we are in your own words.",
    examples: [
      "AZ Off Script is an Arizona creator brand.",
      "We do reactions, group debates, and local hot takes.",
      "It's a room of different women keeping it real, off script.",
    ],
    makeItYours: "Your words, your take. The point is clarity — say what we are so people and search understand.",
    seoPhrase: "what is AZ Off Script",
    captionStarter: "So what is AZ Off Script? Here's the 10-second version.",
    hashtagStarter: ["#AZOffScript", "#ArizonaCreators", "#CreatorCrew", "#ArizonaTikTok", "#WhatIsAZOffScript"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
  },

  // ===== ARIZONA REAL LIFE =====
  {
    id: "before_i_leave_house",
    name: "Before I Leave the House",
    bucket: "Arizona Real Life",
    topicWorld: "Arizona Life",
    description: "What you need before leaving the house in Arizona.",
    effort: "5-Min Drop",
    timeEstimate: "2-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: false,
    maxSeconds: 15,
    idea: "Finish the sentence: 'Before I leave the house in Arizona, I need...'",
    vibe: "Relatable, funny, real. Everyone has their list.",
    whatToDrop: "Your 'before I leave' list. 10-15 seconds.",
    easyWay: "Just say what you actually need before walking out the door.",
    examples: [
      "Before I leave the house, I need water, sunglasses, emotional support, and a reason.",
      "Before I leave the house, I need to know if this errand is really worth melting.",
      "Before I leave the house, somebody better explain why this can't be delivered.",
    ],
    makeItYours: "Your list, your words. The more honest the funnier.",
    seoPhrase: "before I leave the house Arizona",
    captionStarter: "Before I leave the house in Arizona, I need... What's on your list?",
    hashtagStarter: ["#AZOffScript", "#ArizonaLife", "#ArizonaTikTok", "#ArizonaCreators", "#BeforeILeave"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
  },
  {
    id: "kitchen_counter_confessions",
    name: "Kitchen Counter Confessions",
    bucket: "Arizona Real Life",
    topicWorld: "Arizona Life",
    description: "Easy home setup. One take. Just you and the counter.",
    effort: "2-Min Drop",
    timeEstimate: "1-2 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: false,
    maxSeconds: 15,
    idea: "Stand at your kitchen counter and make a confession — something honest and small.",
    vibe: "Casual, real, a little messy. Like you're talking to yourself.",
    whatToDrop: "Your kitchen counter confession. 10-15 seconds.",
    easyWay: "Stand at the counter, hit record, say your confession.",
    examples: [
      "Kitchen counter confession: I'm not going if parking is bad.",
      "Kitchen counter confession: I said 'let me know' and immediately regretted it.",
      "Kitchen counter confession: I do not trust 'quick question.'",
    ],
    makeItYours: "Your confession, your kitchen, your energy. The casual is the point.",
    seoPhrase: "kitchen counter confession",
    captionStarter: "Kitchen counter confession time. What's yours?",
    hashtagStarter: ["#AZOffScript", "#KitchenCounterConfessions", "#ArizonaCreators", "#Confession", "#Relatable"],
    platforms: ["TikTok", "Reels", "Shorts"],
  },

  // ===== WHAT TYPE ARE YOU =====
  {
    id: "which_one_are_you",
    name: "Which One Are You?",
    bucket: "What Type Are You",
    topicWorld: "Friendship",
    description: "Pick which friend type you are. Very shareable.",
    effort: "5-Min Drop",
    timeEstimate: "2-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 15,
    idea: "We list friend types. You pick which one you are.",
    vibe: "Fun, self-aware, shareable. People tag themselves.",
    whatToDrop: "Your pick + why. 10-15 seconds.",
    easyWay: "Just say which one you are and why.",
    examples: [
      "The friend who plans everything.",
      "The friend who shows up late.",
      "The friend who says 'I'm outside' from the couch.",
      "The friend who needs the address 4 times.",
      "The friend who brings snacks.",
      "The friend who gives the face before the words.",
    ],
    makeItYours: "Pick yours or make up your own type. Your personality is the answer.",
    seoPhrase: "which friend are you",
    captionStarter: "Which one are you? Tag yourself. Don't lie.",
    hashtagStarter: ["#AZOffScript", "#WhichOneAreYou", "#ArizonaCreators", "#FriendType", "#Relatable"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
  },
  {
    id: "arizona_which_one",
    name: "Arizona: Which One Are You?",
    bucket: "What Type Are You",
    topicWorld: "Friendship",
    description: "Arizona-specific friend types.",
    effort: "5-Min Drop",
    timeEstimate: "2-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 15,
    idea: "We list Arizona friend types. You pick which one you are.",
    vibe: "Local, funny, self-aware. Arizona-specific.",
    whatToDrop: "Your AZ type pick. 10-15 seconds.",
    easyWay: "Just say which Arizona type you are.",
    examples: [
      "The one who parks far for shade.",
      "The one who refuses outdoor seating.",
      "The one who says 'it's not that hot.'",
      "The one who keeps a water bottle everywhere.",
      "The one who needs AC before conversation.",
    ],
    makeItYours: "Your type, your words. Or make up your own Arizona type.",
    seoPhrase: "Arizona friend type",
    captionStarter: "Which Arizona type are you? Be honest.",
    hashtagStarter: ["#AZOffScript", "#ArizonaLife", "#ArizonaTikTok", "#WhichOneAreYou", "#ArizonaCreators"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
  },

  // ===== QUIET GIRL FRIENDLY =====
  {
    id: "no_words_needed",
    name: "No Words Needed",
    bucket: "Quiet Girl Friendly",
    topicWorld: "Arizona Life",
    description: "Face reactions only — perfect for quiet personalities.",
    effort: "2-Min Drop",
    timeEstimate: "1-2 min",
    homeFriendly: true,
    needsTalking: false,
    needsEditing: false,
    adminStitches: false,
    maxSeconds: 10,
    idea: "Your face when you hear the prompt. No talking required.",
    vibe: "Quiet, real, powerful. Your face is enough.",
    whatToDrop: "Your face reaction. 5-10 seconds. No words.",
    easyWay: "Read the prompt, hit record, give your honest face.",
    examples: [
      "Your face when someone says they're five minutes away.",
      "Your face when the group chat says 'be honest.'",
      "Your face when someone says Arizona summer is not that bad.",
      "Your face when someone says 'quick favor.'",
      "Your face when plans move from indoors to outside.",
    ],
    makeItYours: "Your face, your reaction. No performing. This helps quiet personalities shine.",
    seoPhrase: "face reaction no words",
    captionStarter: "No words needed. Just the face. What's yours?",
    hashtagStarter: ["#AZOffScript", "#NoWordsNeeded", "#FaceReaction", "#ArizonaCreators", "#QuietGirlEnergy"],
    platforms: ["TikTok", "Reels", "Shorts"],
  },

  // ===== OFF SCRIPT LOOKS =====
  {
    id: "off_script_looks",
    name: "Off Script Looks",
    bucket: "Off Script Looks",
    topicWorld: "Beauty / Style",
    description: "Outfit, style, merch — polished but still real.",
    effort: "10-Min Drop",
    timeEstimate: "5-10 min",
    homeFriendly: true,
    needsTalking: false,
    needsEditing: true,
    adminStitches: true,
    maxSeconds: 15,
    idea: "Show your fit — what you'd wear to an AZ Off Script content day.",
    vibe: "Cool, styled, confident. Less goofy, more vibe.",
    whatToDrop: "A fit check or outfit reveal. 10-15 seconds.",
    easyWay: "Just show your outfit — full look, accessories, tumbler if you have one.",
    examples: [
      "What I'd wear to an AZ Off Script content day.",
      "Black shirt or white shirt?",
      "Tumbler check.",
      "Arizona, our way — outfit edition.",
      "Crew fit check.",
    ],
    makeItYours: "Your style, your fit. This can show merch without hard-selling.",
    seoPhrase: "Arizona creator outfit",
    captionStarter: "Off Script fit check. How would you style it?",
    hashtagStarter: ["#AZOffScript", "#FitCheck", "#ArizonaCreators", "#OffScriptLooks", "#ArizonaStyle"],
    platforms: ["TikTok", "Reels", "Shorts"],
  },

  // ===== REAL BUT NOT TOO DEEP =====
  {
    id: "real_quick",
    name: "Real Quick",
    bucket: "Real But Not Too Deep",
    topicWorld: "Soft Truths",
    description: "One real thought, delivered quick. Gives the brand heart.",
    effort: "5-Min Drop",
    timeEstimate: "2-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: false,
    maxSeconds: 15,
    idea: "Share one real thought — quick, honest, not a whole speech.",
    vibe: "Real, warm, brief. Use sparingly. Gives dimension.",
    whatToDrop: "Your 'real quick' thought. 10-15 seconds.",
    easyWay: "Just say one real thing. Don't make it a speech.",
    examples: [
      "Real quick: adult friendships are harder than people admit.",
      "Real quick: sometimes the group chat is therapy and chaos.",
      "Real quick: you can be low-key and still have presence.",
      "Real quick: not every woman wants to be loud, and that's fine.",
      "Real quick: showing up is a love language too.",
    ],
    makeItYours: "Your thought, your words. Keep it short. The real is the point.",
    seoPhrase: "real quick honest thought",
    captionStarter: "Real quick... what's one thing you've learned?",
    hashtagStarter: ["#AZOffScript", "#RealQuick", "#ArizonaCreators", "#Honest", "#RealTalk"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    shotRecipeId: "recipe_real_quick_b",
  },

  // ===== FAKE SCENARIO COURT =====
  {
    id: "fake_scenario_court",
    name: "Fake Scenario Court",
    bucket: "Fake Scenario Court",
    topicWorld: "Dating",
    description: "Fake situations, real reactions. Drama energy without real drama.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 20,
    idea: "React to a fake scenario — give your verdict on a made-up situation.",
    vibe: "Drama energy, but safe. Fake situations, real reactions.",
    whatToDrop: "Your reaction to the fake scenario. 15-20 seconds.",
    easyWay: "Just react to the scenario like it's real. Your honest take.",
    examples: [
      "Fake scenario: Your friend says she's not mad, then sends 'k.'",
      "Fake scenario: Someone invites the group but leaves one person out.",
      "Fake scenario: He says he forgot to text back, but posted three stories.",
      "Fake scenario: Your friend says 'be honest' but clearly wants agreement.",
    ],
    makeItYours: "Your reaction, your verdict. The scenario is fake — your take is real.",
    seoPhrase: "fake scenario reaction",
    captionStarter: "Fake scenario, real reaction. What would you do?",
    hashtagStarter: ["#AZOffScript", "#FakeScenario", "#ReactionVideo", "#ArizonaCreators", "#WhatWouldYouDo"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
  },

  // ===== ARIZONA WOMAN MATH =====
  {
    id: "arizona_woman_math",
    name: "Arizona Woman Math",
    bucket: "Arizona Woman Math",
    topicWorld: "Arizona Life",
    description: "Funny Arizona logic. Don't overuse.",
    effort: "5-Min Drop",
    timeEstimate: "2-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: false,
    maxSeconds: 15,
    idea: "Finish the sentence: 'Arizona woman math: ...' with your AZ logic.",
    vibe: "Funny, relatable, a little chaotic. Arizona-specific logic.",
    whatToDrop: "Your 'Arizona woman math' line. 10-15 seconds.",
    easyWay: "Just say your Arizona logic. The more absurd the better.",
    examples: [
      "Arizona woman math: If the parking spot has shade, it counts as closer.",
      "Arizona woman math: A 30-minute drive is only acceptable if food is involved.",
      "Arizona woman math: If it is over 105, the outfit has to be cute AND survivable.",
      "Arizona woman math: If I already took my bra off, I'm not leaving again.",
    ],
    makeItYours: "Your math, your logic. The more specific the funnier. Don't overuse this one.",
    seoPhrase: "Arizona woman math",
    captionStarter: "Arizona woman math. What's your equation?",
    hashtagStarter: ["#AZOffScript", "#ArizonaWomanMath", "#ArizonaTikTok", "#ArizonaCreators", "#ArizonaLife"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
  },

  // ===== TEXT TRANSLATION =====
  {
    id: "what_she_really_means",
    name: "What She Really Means",
    bucket: "Text Translation",
    topicWorld: "Texting",
    description: "Translate common phrases. Easy, searchable, relatable.",
    effort: "5-Min Drop",
    timeEstimate: "2-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 15,
    idea: "We give you a phrase. You translate what it really means.",
    vibe: "Funny, honest, relatable. Everyone knows these phrases.",
    whatToDrop: "Your translation of the phrase. 10-15 seconds.",
    easyWay: "Just say what the phrase actually means.",
    examples: [
      "When she says 'it's fine.'",
      "When she says 'do whatever you want.'",
      "When she says 'I'm not mad.'",
      "When she says 'we'll see.'",
      "When she says 'I'll let you know.'",
    ],
    makeItYours: "Your translation, your delivery. The honest read is the funny one.",
    seoPhrase: "what she really means text",
    captionStarter: "When she says 'it's fine'... what does she really mean?",
    hashtagStarter: ["#AZOffScript", "#WhatSheReallyMeans", "#ArizonaCreators", "#TextTranslation", "#Relatable"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
  },

  // ===== RANK IT =====
  {
    id: "rank_it",
    name: "Rank It",
    bucket: "Rank It",
    topicWorld: "Arizona Life",
    description: "Rank a list of Arizona problems. Hold up fingers or say the number.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 20,
    idea: "We give you a list. You rank it from worst to least worst.",
    vibe: "Quick, opinionated, visual. Hold up fingers or say the number.",
    whatToDrop: "Your ranking. 15-20 seconds.",
    easyWay: "Just hold up fingers or say the number for each item.",
    examples: [
      "Rank these Arizona problems: hot car seat, steering wheel burn, bad parking, long drive, outside plans, no shade.",
      "Rank these group chat problems: voice notes, 'k', read receipts, no reply, typing but never sending.",
    ],
    makeItYours: "Your ranking, your logic. The ranking is the opinion.",
    seoPhrase: "rank it Arizona problems",
    captionStarter: "We ranked these Arizona problems. Do you agree?",
    hashtagStarter: ["#AZOffScript", "#RankIt", "#ArizonaTikTok", "#ArizonaCreators", "#ArizonaProblems"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
  },

  // ===== ROOM TEMPERATURE CHECK =====
  {
    id: "room_temperature_check",
    name: "Room Temperature Check",
    bucket: "Room Temperature Check",
    topicWorld: "Arizona Life",
    description: "Not weather — vibe. A recurring branded segment.",
    effort: "5-Min Drop",
    timeEstimate: "2-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 15,
    idea: "We ask a vibe question. You give the room temperature — your honest read.",
    vibe: "Branded, recurring, quick. 'Room temperature check' is the signature.",
    whatToDrop: "Your room temperature check answer. 10-15 seconds.",
    easyWay: "Just answer the vibe question honestly.",
    examples: [
      "Room temperature check: Would you still go if the plans changed last minute?",
      "Room temperature check: Is this rude or honest?",
      "Room temperature check: Are we forgiving this or not?",
    ],
    makeItYours: "Your read, your energy. This can become a recurring branded segment.",
    seoPhrase: "room temperature check",
    captionStarter: "Room temperature check: what's the read?",
    hashtagStarter: ["#AZOffScript", "#RoomTemperatureCheck", "#ArizonaCreators", "#ReactionVideo", "#GroupReaction"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
  },

  // ===== DON'T OVERTHINK IT =====
  {
    id: "dont_overthink_it",
    name: "Don't Overthink It",
    bucket: "Don't Overthink It",
    topicWorld: "Arizona Life",
    description: "One prompt, answer fast. First answer wins.",
    effort: "2-Min Drop",
    timeEstimate: "1-2 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: false,
    maxSeconds: 10,
    idea: "We give you one prompt. Answer fast — first answer wins.",
    vibe: "Quick, instinctive, real. No thinking, just answering.",
    whatToDrop: "Your first answer. 5-10 seconds.",
    easyWay: "Just say the first thing that comes to mind.",
    examples: [
      "One word for Arizona summer.",
      "One thing you refuse to argue about.",
      "One thing that instantly changes your mood.",
      "One thing every group chat needs.",
      "One thing you are too grown to explain.",
    ],
    makeItYours: "Your first answer. Don't edit yourself. The card says: Don't overthink it. First answer wins.",
    seoPhrase: "don't overthink it quick answer",
    captionStarter: "Don't overthink it. First answer wins. What's yours?",
    hashtagStarter: ["#AZOffScript", "#DontOverthinkIt", "#ArizonaCreators", "#QuickAnswer", "#FirstAnswerWins"],
    platforms: ["TikTok", "Reels", "Shorts"],
  },

  // ===== TWO-SECOND OPINION =====
  {
    id: "two_second_opinion",
    name: "Two-Second Opinion",
    bucket: "Two-Second Opinion",
    topicWorld: "Arizona Life",
    description: "Yes or no. That's it. Super easy weekly format.",
    effort: "2-Min Drop",
    timeEstimate: "1-2 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 5,
    idea: "We give you a question. You say yes or no. That's it.",
    vibe: "Fast, final, easy. One of the easiest weekly formats.",
    whatToDrop: "Your yes or no. 2-5 seconds.",
    easyWay: "Just say yes or no. Don't explain.",
    examples: [
      "Would you go? Yes or no.",
      "Would you forgive it? Yes or no.",
      "Worth the drive? Yes or no.",
      "Would you tell her? Yes or no.",
      "Would you post it? Yes or no.",
    ],
    makeItYours: "Your answer. One word. The brevity is the content.",
    seoPhrase: "yes or no opinion video",
    captionStarter: "Yes or no. That's it. What's your answer?",
    hashtagStarter: ["#AZOffScript", "#YesOrNo", "#ArizonaCreators", "#QuickOpinion", "#TwoSecondOpinion"],
    platforms: ["TikTok", "Reels", "Shorts"],
  },

  // ===== DIFFERENT WOMEN DIFFERENT ANSWER =====
  {
    id: "different_women_different_answer",
    name: "Different Women, Different Answer",
    bucket: "Different Women Different Answer",
    topicWorld: "Crew / Future Wave",
    description: "Same prompt, totally different responses. Shows the range of the room.",
    effort: "10-Min Drop",
    timeEstimate: "5-10 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: true,
    adminStitches: true,
    maxSeconds: 20,
    idea: "Same prompt, but everyone answers differently. That's the point.",
    vibe: "Diverse, real, shows range. The differences are the content.",
    whatToDrop: "Your answer to the prompt. 15-20 seconds.",
    easyWay: "Just answer the prompt your way. Don't match anyone else.",
    examples: [
      "Prompt: What makes you leave the group chat? (One says drama, one says notifications, one says voice notes.)",
      "Prompt: What's your Arizona dealbreaker? (Everyone has a different one.)",
      "Prompt: What makes you trust someone? (Different answers show different personalities.)",
    ],
    makeItYours: "Your answer, your personality. The point is NOT to match. Different women, different answers.",
    seoPhrase: "different women different answers",
    captionStarter: "Same prompt, totally different answers. Which one is yours?",
    hashtagStarter: ["#AZOffScript", "#DifferentWomen", "#ArizonaCreators", "#GroupReaction", "#SamePrompt"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
  },

  // ===== CREW / FUTURE WAVE =====
  {
    id: "first_wave_vs_future",
    name: "Crew vs Future Wave",
    bucket: "Crew / Future Wave",
    topicWorld: "Crew / Future Wave",
    description: "The crew reacts to future member questions. Helps attract new women.",
    effort: "10-Min Drop",
    timeEstimate: "5-10 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: true,
    adminStitches: true,
    maxSeconds: 20,
    idea: "Answer questions about what kind of woman fits AZ Off Script and what the room needs next.",
    vibe: "Welcoming, honest, forward-looking. Helps with recruitment.",
    whatToDrop: "Your answer to the future wave question. 15-20 seconds.",
    easyWay: "Just answer honestly about what the room needs.",
    examples: [
      "What kind of woman fits AZ Off Script?",
      "What should someone know before joining the room?",
      "What energy does the room still need?",
    ],
    makeItYours: "Your honest answer. This helps attract more women without sounding desperate.",
    seoPhrase: "join Arizona creator crew",
    captionStarter: "The room is growing. What kind of energy should walk in next?",
    hashtagStarter: ["#AZOffScript", "#FirstWave", "#FutureWave", "#ArizonaCreators", "#JoinTheCrew", "#CreatorCrew"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
  },

  // ===== BLACK + LATINA LENS (optional, natural) =====
  {
    id: "black_latina_lens",
    name: "Black + Latina Lens",
    bucket: "Black + Latina Lens",
    topicWorld: "Crew / Future Wave",
    description: "Cultural takes — natural, not performed. Optional category, not the whole brand.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 20,
    idea: "Answer a cultural prompt as yourself — not performing, just being.",
    vibe: "Real, warm, honest. No stereotypes. No performing culture.",
    whatToDrop: "Your take on the prompt. 15-20 seconds.",
    easyWay: "Just answer like you're talking to your family or your group chat.",
    examples: [
      "What would your auntie/tía say?",
      "Family group chat translation.",
      "The look your mom gives before she says something wild.",
    ],
    makeItYours: "Answer as yourself. Do not force accents, stereotypes, or 'performing culture.' Your real voice is the point. This is an optional category — use when it fits naturally.",
    seoPhrase: "Black Latina women creators Arizona",
    captionStarter: "The auntie would say... What would yours say?",
    hashtagStarter: ["#AZOffScript", "#BlackWomenCreators", "#LatinaCreators", "#ArizonaWomen", "#ArizonaCreators"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
  },

  // ===== MEN, WOMEN & THE SCRIPT =====
  {
    id: "he_said_she_heard",
    name: "He Said / She Heard",
    bucket: "Men, Women & the Script",
    topicWorld: "Men, Women & the Script",
    description: "One person reads what he said. The room reacts to what women actually heard.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 15,
    idea: "React to a phrase a man said — and tell us what women actually heard.",
    vibe: "Funny, honest, instantly debatable. Everyone has heard these lines.",
    whatToDrop: "Your reaction to the phrase + what you actually heard. 10-15 seconds.",
    easyWay: "Read the phrase out loud, then give your honest translation.",
    examples: [
      "He said he's just bad at texting.",
      "He said he doesn't like labels.",
      "He said he forgot.",
      "He said you're overthinking.",
      "He said he was going to do it later.",
    ],
    makeItYours: "Your reaction, your translation. The honest read is the funny one.",
    seoPhrase: "he said she heard reaction",
    captionStarter: "He said one thing. We heard another. What did you hear?",
    hashtagStarter: ["#AZOffScript", "#HeSaidSheHeard", "#DatingScripts", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "One person reads what he said. The room reacts to what women actually heard.",
    whatEachPersonDoes: "Record your face or one-line reaction to the phrase.",
    whatToSend: "A short clip reacting to the phrase + your translation. 10-15 seconds.",
    examplePrompts: [
      "He said he's just bad at texting.",
      "He said he doesn't like labels.",
      "He said he forgot.",
      "He said you're overthinking.",
      "He said he was going to do it later.",
    ],
    whyItWorks: "People will argue in the comments because everyone has heard these lines differently.",
    bestFor: "Talkers and expressive faces. Group stitch energy.",
    shotRecipeId: "recipe_he_said_she_heard_b",
  },
  {
    id: "helping_or_making_worse",
    name: "Helping or Making It Worse?",
    bucket: "Men, Women & the Script",
    topicWorld: "Men, Women & the Script",
    description: "The room decides if something is actually helpful or just creating more work.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 15,
    idea: "We give you a scenario. You decide: was that actually helping or did it make more work?",
    vibe: "Funny, relatable, a little exasperated. The mental load is real.",
    whatToDrop: "Your verdict on the scenario. 10-15 seconds.",
    easyWay: "Just react to the scenario and say if it helped or made it worse.",
    examples: [
      "He watched the kids but left the whole kitchen destroyed.",
      "He said 'just tell me what to do.'",
      "He cleaned but moved everything to the wrong place.",
      "He says he helps, but only after being asked five times.",
    ],
    makeItYours: "Your verdict, your reasoning. Call it like you see it.",
    seoPhrase: "helping or making it worse reaction",
    captionStarter: "Did that help or make it worse? Be honest.",
    hashtagStarter: ["#AZOffScript", "#HelpingOrWorse", "#MentalLoad", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "The room decides if something is actually helpful or just creating more work.",
    whatEachPersonDoes: "React to the scenario and give your verdict.",
    whatToSend: "Your verdict + quick reason. 10-15 seconds.",
    examplePrompts: [
      "He watched the kids but left the whole kitchen destroyed.",
      "He said 'just tell me what to do.'",
      "He cleaned but moved everything to the wrong place.",
      "He says he helps, but only after being asked five times.",
    ],
    whyItWorks: "Mom-life and relationship humor. Everyone has lived this.",
    bestFor: "Mom-life, relationship humor, women with expressive reactions.",
  },
  {
    id: "caring_or_controlling",
    name: "Caring or Controlling?",
    bucket: "Men, Women & the Script",
    topicWorld: "Men, Women & the Script",
    description: "The room decides if a behavior is sweet, protective, controlling, or suspicious.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 15,
    idea: "We give you a behavior. You decide: caring, protective, controlling, or suspicious?",
    vibe: "Debate energy. Instant comments. People will argue hard.",
    whatToDrop: "Your verdict on the behavior. 10-15 seconds.",
    easyWay: "Just say which one it is and why.",
    examples: [
      "He wants your location on.",
      "He asks what you're wearing before you leave.",
      "He says he doesn't like your friend.",
      "He checks if you made it home.",
      "He wants passwords.",
    ],
    makeItYours: "Your verdict, your reasoning. The line between caring and controlling is the debate.",
    seoPhrase: "caring or controlling relationship reaction",
    captionStarter: "Caring or controlling? You decide.",
    hashtagStarter: ["#AZOffScript", "#CaringOrControlling", "#DatingScripts", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "The room decides if a behavior is sweet, protective, controlling, or suspicious.",
    whatEachPersonDoes: "Hear the behavior, give your verdict.",
    whatToSend: "Your verdict + quick reason. 10-15 seconds.",
    examplePrompts: [
      "He wants your location on.",
      "He asks what you're wearing before you leave.",
      "He says he doesn't like your friend.",
      "He checks if you made it home.",
      "He wants passwords.",
    ],
    whyItWorks: "Instant comments. People will debate hard.",
    bestFor: "Talkers, expressive faces, group stitch.",
    shotRecipeId: "recipe_caring_controlling_b",
  },
  {
    id: "men_think_means_nothing",
    name: "Men Think This Means Nothing",
    bucket: "Men, Women & the Script",
    topicWorld: "Men, Women & the Script",
    description: "The room reacts to things men may think are small but women clock immediately.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 15,
    idea: "React to something men think is no big deal — but women notice instantly.",
    vibe: "Sharp, funny, knowing. The 'we all see it' energy.",
    whatToDrop: "Your reaction to the thing. 10-15 seconds.",
    easyWay: "Just react and say why it's not nothing.",
    examples: [
      "Leaving you on read.",
      "Changing tone over text.",
      "Following random girls.",
      "Saying 'do what you want.'",
      "Not planning the date.",
    ],
    makeItYours: "Your reaction, your read. The 'we all clocked it' energy is the content.",
    seoPhrase: "men think this means nothing women notice",
    captionStarter: "He thinks that meant nothing. We noticed immediately.",
    hashtagStarter: ["#AZOffScript", "#MenThinkNothing", "#DatingScripts", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "The room reacts to things men may think are small but women clock immediately.",
    whatEachPersonDoes: "React to the thing and say why it's not nothing.",
    whatToSend: "Your reaction. 10-15 seconds.",
    examplePrompts: [
      "Leaving you on read.",
      "Changing tone over text.",
      "Following random girls.",
      "Saying 'do what you want.'",
      "Not planning the date.",
    ],
    whyItWorks: "Women will tag their friends. Men will argue. Both drive engagement.",
    bestFor: "Talkers and expressive faces.",
  },

  // ===== DATING SCRIPTS =====
  {
    id: "red_flag_real_life_just_tired",
    name: "Red Flag, Real Life, or Just Tired?",
    bucket: "Dating Scripts",
    topicWorld: "Dating",
    description: "Not everything is toxic. Some things are just adult life. The room decides.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 15,
    idea: "We give you a behavior. You decide: red flag, real life, or just tired?",
    vibe: "Funny, mature, balanced. Not everything is toxic.",
    whatToDrop: "Your verdict on the behavior. 10-15 seconds.",
    easyWay: "Just say which one it is and why.",
    examples: [
      "He doesn't text much during work.",
      "She doesn't want to go out after 8.",
      "He forgot the exact date but remembered the month.",
      "She needs three business days to respond.",
      "He says he's tired every weekend.",
    ],
    makeItYours: "Your verdict, your reasoning. Keep it funny and mature.",
    seoPhrase: "red flag or just tired dating reaction",
    captionStarter: "Red flag, real life, or just tired? You decide.",
    hashtagStarter: ["#AZOffScript", "#RedFlagOrRealLife", "#DatingScripts", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "Not everything is toxic. Some things are just adult life. The room decides.",
    whatEachPersonDoes: "Hear the behavior, give your verdict.",
    whatToSend: "Your verdict + quick reason. 10-15 seconds.",
    examplePrompts: [
      "He doesn't text much during work.",
      "She doesn't want to go out after 8.",
      "He forgot the exact date but remembered the month.",
      "She needs three business days to respond.",
      "He says he's tired every weekend.",
    ],
    whyItWorks: "It keeps the brand funny and mature, not just 'everything is a red flag.'",
    bestFor: "Talkers, group stitch, solo home clips.",
    shotRecipeId: "recipe_red_flag_tired_b",
  },
  {
    id: "apology_or_excuse",
    name: "Apology or Excuse?",
    bucket: "Dating Scripts",
    topicWorld: "Dating",
    description: "Read a fake apology. The room decides if it counts.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 15,
    idea: "We give you a fake apology. You decide: real apology or just an excuse?",
    vibe: "Sharp, funny, honest. Everyone has heard these.",
    whatToDrop: "Your verdict on the apology. 10-15 seconds.",
    easyWay: "Just say if it counts or not.",
    examples: [
      "I'm sorry you felt that way.",
      "I didn't mean it like that.",
      "I was just joking.",
      "You know how I am.",
      "I said sorry already.",
    ],
    makeItYours: "Your verdict, your read. The fake apology is the prompt — your reaction is the content.",
    seoPhrase: "apology or excuse reaction video",
    captionStarter: "Apology or excuse? Does that count?",
    hashtagStarter: ["#AZOffScript", "#ApologyOrExcuse", "#DatingScripts", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "Read a fake apology. The room decides if it counts.",
    whatEachPersonDoes: "Hear the apology, give your verdict.",
    whatToSend: "Your verdict + quick reason. 10-15 seconds.",
    examplePrompts: [
      "I'm sorry you felt that way.",
      "I didn't mean it like that.",
      "I was just joking.",
      "You know how I am.",
      "I said sorry already.",
    ],
    whyItWorks: "Everyone has received a fake apology. Instant relatability.",
    bestFor: "Talkers, expressive faces, group stitch.",
    shotRecipeId: "recipe_apology_or_excuse_b",
  },
  {
    id: "would_you_tell_your_friend",
    name: "Would You Tell Your Friend?",
    bucket: "Dating Scripts",
    topicWorld: "Dating",
    description: "A situation is read out loud. The room decides whether to tell the friend, stay quiet, or mind your business.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 20,
    idea: "We give you a situation. You decide: tell your friend, stay quiet, or mind your business?",
    vibe: "Drama energy, but thoughtful. The girl code debate.",
    whatToDrop: "Your verdict on the situation. 15-20 seconds.",
    easyWay: "Just say what you'd do and why.",
    examples: [
      "You saw her man liking every picture.",
      "You heard he was out with someone else.",
      "Her friend is talking about her.",
      "Her boyfriend said something weird to you.",
      "You know the gift was not his idea.",
    ],
    makeItYours: "Your verdict, your reasoning. The girl code debate is the content.",
    seoPhrase: "would you tell your friend girl code",
    captionStarter: "Would you tell your friend? Or mind your business?",
    hashtagStarter: ["#AZOffScript", "#WouldYouTell", "#GirlCode", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "A situation is read out loud. The room decides whether to tell the friend, stay quiet, or mind your business.",
    whatEachPersonDoes: "Hear the situation, give your verdict.",
    whatToSend: "Your verdict + quick reason. 15-20 seconds.",
    examplePrompts: [
      "You saw her man liking every picture.",
      "You heard he was out with someone else.",
      "Her friend is talking about her.",
      "Her boyfriend said something weird to you.",
      "You know the gift was not his idea.",
    ],
    whyItWorks: "Girl code debates always go viral. People will argue in comments.",
    bestFor: "Talkers, group stitch, solo home clips.",
  },

  // ===== TEXTING SCRIPTS =====
  {
    id: "text_translation_v2",
    name: "Text Translation",
    bucket: "Texting Scripts",
    topicWorld: "Texting",
    description: "Translate common texts into what they really mean.",
    effort: "5-Min Drop",
    timeEstimate: "2-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 15,
    idea: "We give you a text. You translate what it really means.",
    vibe: "Funny, honest, relatable. Everyone knows these texts.",
    whatToDrop: "Your translation of the text. 10-15 seconds.",
    easyWay: "Just say what the text actually means.",
    examples: [
      "K.",
      "Do what you want.",
      "I'm not mad.",
      "We'll see.",
      "Lol.",
      "It's fine.",
    ],
    makeItYours: "Your translation, your delivery. The honest read is the funny one.",
    seoPhrase: "text translation what it really means",
    captionStarter: "When they say 'it's fine'... what does it really mean?",
    hashtagStarter: ["#AZOffScript", "#TextTranslation", "#TextingScripts", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "Translate common texts into what they really mean.",
    whatEachPersonDoes: "Say the translation or just react with your face.",
    whatToSend: "Your translation of the text. 10-15 seconds.",
    examplePrompts: [
      "K.",
      "Do what you want.",
      "I'm not mad.",
      "We'll see.",
      "Lol.",
      "It's fine.",
    ],
    whyItWorks: "Everyone has received these texts. Instant relatability and shareability.",
    bestFor: "Talkers and quiet faces. Face-only reaction works too.",
  },
  {
    id: "too_fast_too_slow_normal",
    name: "Too Fast, Too Slow, or Normal?",
    bucket: "Texting Scripts",
    topicWorld: "Texting",
    description: "The room judges texting behavior.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 15,
    idea: "We give you a texting behavior. You judge: too fast, too slow, or normal?",
    vibe: "Quick, opinionated, funny. Everyone has a texting opinion.",
    whatToDrop: "Your verdict on the texting behavior. 10-15 seconds.",
    easyWay: "Just say which one it is and why.",
    examples: [
      "Replying in 30 seconds.",
      "Taking 8 hours.",
      "Sending paragraphs.",
      "Only texting after midnight.",
      "Watching stories but not replying.",
    ],
    makeItYours: "Your verdict, your reasoning. The texting debate is the content.",
    seoPhrase: "texting too fast too slow reaction",
    captionStarter: "Too fast, too slow, or normal? Judge the texting.",
    hashtagStarter: ["#AZOffScript", "#TooFastTooSlow", "#TextingScripts", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "The room judges texting behavior.",
    whatEachPersonDoes: "Hear the texting behavior, give your verdict.",
    whatToSend: "Your verdict + quick reason. 10-15 seconds.",
    examplePrompts: [
      "Replying in 30 seconds.",
      "Taking 8 hours.",
      "Sending paragraphs.",
      "Only texting after midnight.",
      "Watching stories but not replying.",
    ],
    whyItWorks: "Everyone has an opinion on texting. Instant debate.",
    bestFor: "Talkers, group stitch, solo home clips.",
  },

  // ===== FRIENDSHIP COURT =====
  {
    id: "friend_or_follower",
    name: "Friend or Follower?",
    bucket: "Friendship Court",
    topicWorld: "Friendship",
    description: "The room decides if someone is a real friend or just watching your life.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 15,
    idea: "We give you a behavior. You decide: real friend or just a follower?",
    vibe: "Honest, a little spicy. The 'we all know one' energy.",
    whatToDrop: "Your verdict on the behavior. 10-15 seconds.",
    easyWay: "Just say which one it is and why.",
    examples: [
      "She watches every story but never checks on you.",
      "She only comments when things look good.",
      "She disappears when you're struggling.",
      "She supports strangers louder than you.",
      "She says 'we should hang out' but never picks a day.",
    ],
    makeItYours: "Your verdict, your reasoning. Call it like you see it.",
    seoPhrase: "friend or follower reaction video",
    captionStarter: "Friend or follower? Be honest.",
    hashtagStarter: ["#AZOffScript", "#FriendOrFollower", "#FriendshipCourt", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "The room decides if someone is a real friend or just watching your life.",
    whatEachPersonDoes: "Hear the behavior, give your verdict.",
    whatToSend: "Your verdict + quick reason. 10-15 seconds.",
    examplePrompts: [
      "She watches every story but never checks on you.",
      "She only comments when things look good.",
      "She disappears when you're struggling.",
      "She supports strangers louder than you.",
      "She says 'we should hang out' but never picks a day.",
    ],
    whyItWorks: "Everyone has a friend-or-follower. People will tag themselves.",
    bestFor: "Talkers, expressive faces, group stitch.",
    shotRecipeId: "recipe_friend_or_follower_b",
  },
  {
    id: "girl_code_or_just_drama",
    name: "Girl Code or Just Drama?",
    bucket: "Friendship Court",
    topicWorld: "Friendship",
    description: "The room decides if something broke girl code or if people are doing too much.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 15,
    idea: "We give you a situation. You decide: broke girl code or people doing too much?",
    vibe: "Debate energy. The girl code line is the content.",
    whatToDrop: "Your verdict on the situation. 10-15 seconds.",
    easyWay: "Just say which one it is and why.",
    examples: [
      "Dating someone your friend talked to once.",
      "Posting the same outfit.",
      "Not telling your friend what you heard.",
      "Inviting someone your friend doesn't like.",
      "Still being cool with someone who hurt your friend.",
    ],
    makeItYours: "Your verdict, your reasoning. The girl code debate is the content.",
    seoPhrase: "girl code or just drama reaction",
    captionStarter: "Girl code or just drama? You decide.",
    hashtagStarter: ["#AZOffScript", "#GirlCodeOrDrama", "#FriendshipCourt", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "The room decides if something broke girl code or if people are doing too much.",
    whatEachPersonDoes: "Hear the situation, give your verdict.",
    whatToSend: "Your verdict + quick reason. 10-15 seconds.",
    examplePrompts: [
      "Dating someone your friend talked to once.",
      "Posting the same outfit.",
      "Not telling your friend what you heard.",
      "Inviting someone your friend doesn't like.",
      "Still being cool with someone who hurt your friend.",
    ],
    whyItWorks: "Girl code debates always go viral. Comments will be a war zone.",
    bestFor: "Talkers, group stitch, solo home clips.",
    shotRecipeId: "recipe_girl_code_or_just_drama_b",
  },
  {
    id: "friend_every_group_has",
    name: "The Friend Every Group Has",
    bucket: "Friendship Court",
    topicWorld: "Friendship",
    description: "Each person claims or points to a friend type.",
    effort: "5-Min Drop",
    timeEstimate: "2-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 15,
    idea: "We list friend types. You claim which one you are — or call out someone in the crew.",
    vibe: "Fun, self-aware, shareable. People tag themselves.",
    whatToDrop: "Your pick + why. 10-15 seconds.",
    easyWay: "Just say which one you are and why.",
    examples: [
      "The late friend.",
      "The planner.",
      "The one who cancels but means well.",
      "The one who knows everybody's business.",
      "The one who says 'I'm outside' but is still home.",
      "The one who starts the debate then disappears.",
    ],
    makeItYours: "Pick yours or make up your own type. Your personality is the answer.",
    seoPhrase: "the friend every group has",
    captionStarter: "Which friend are you? Tag yourself. Don't lie.",
    hashtagStarter: ["#AZOffScript", "#FriendEveryGroup", "#FriendshipCourt", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "Each person claims or points to a friend type.",
    whatEachPersonDoes: "Say which friend type you are and why.",
    whatToSend: "Your pick + why. 10-15 seconds.",
    examplePrompts: [
      "The late friend.",
      "The planner.",
      "The one who cancels but means well.",
      "The one who knows everybody's business.",
      "The one who says 'I'm outside' but is still home.",
      "The one who starts the debate then disappears.",
    ],
    whyItWorks: "People will tag themselves and their friends. High shareability.",
    bestFor: "Talkers, group stitch, solo home clips.",
    shotRecipeId: "recipe_friend_every_group_b",
  },

  // ===== MOM-LIFE OFF SCRIPT =====
  {
    id: "mom_math",
    name: "Mom Math",
    bucket: "Mom-Life Off Script",
    topicWorld: "Mom-Life",
    description: "Funny mom logic that only makes sense when you're tired.",
    effort: "5-Min Drop",
    timeEstimate: "2-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: false,
    maxSeconds: 15,
    idea: "Finish the sentence: 'Mom math: ...' with your tired mom logic.",
    vibe: "Funny, exhausted, real. The logic only makes sense if you're a mom.",
    whatToDrop: "Your 'mom math' line. 10-15 seconds.",
    easyWay: "Just say your mom logic. The more absurd the better.",
    examples: [
      "If I sit down for five minutes, that counts as rest.",
      "If the kids ate fruit snacks, that was fruit.",
      "If I bought it for the house, it was not shopping.",
      "If I forgot one thing, we are not going back.",
      "If everyone is alive, I did enough.",
    ],
    makeItYours: "Your math, your logic. The more specific the funnier.",
    seoPhrase: "mom math funny logic",
    captionStarter: "Mom math. What's your equation?",
    hashtagStarter: ["#AZOffScript", "#MomMath", "#MomLifeOffScript", "#ArizonaCreators", "#MomTok"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "Funny mom logic that only makes sense when you're tired.",
    whatEachPersonDoes: "Say your 'mom math' logic.",
    whatToSend: "Your 'mom math' line. 10-15 seconds.",
    examplePrompts: [
      "If I sit down for five minutes, that counts as rest.",
      "If the kids ate fruit snacks, that was fruit.",
      "If I bought it for the house, it was not shopping.",
      "If I forgot one thing, we are not going back.",
      "If everyone is alive, I did enough.",
    ],
    whyItWorks: "Moms will tag each other. Exhausted logic is universally relatable.",
    bestFor: "Moms, solo home clips, talkers.",
    shotRecipeId: "recipe_mom_math_b",
  },
  {
    id: "who_was_supposed_to_do_that",
    name: "Who Was Supposed to Do That?",
    bucket: "Mom-Life Off Script",
    topicWorld: "Mom-Life",
    description: "The room reacts to invisible labor and household responsibility.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 15,
    idea: "We ask who does something invisible in the house. You react.",
    vibe: "Exasperated, funny, real. The mental load is the content.",
    whatToDrop: "Your reaction to the invisible labor question. 10-15 seconds.",
    easyWay: "Just react honestly.",
    examples: [
      "Who makes the appointments?",
      "Who knows the kids' shoe sizes?",
      "Who remembers picture day?",
      "Who notices the toilet paper is gone?",
      "Who buys the birthday gifts?",
    ],
    makeItYours: "Your reaction, your truth. The mental load is the content.",
    seoPhrase: "invisible labor mental load mom reaction",
    captionStarter: "Who was supposed to do that? Be honest.",
    hashtagStarter: ["#AZOffScript", "#InvisibleLabor", "#MomLifeOffScript", "#ArizonaCreators", "#MentalLoad"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "The room reacts to invisible labor and household responsibility.",
    whatEachPersonDoes: "React to the question of who does the invisible work.",
    whatToSend: "Your reaction. 10-15 seconds.",
    examplePrompts: [
      "Who makes the appointments?",
      "Who knows the kids' shoe sizes?",
      "Who remembers picture day?",
      "Who notices the toilet paper is gone?",
      "Who buys the birthday gifts?",
    ],
    whyItWorks: "Every household has this dynamic. Instant relatability.",
    bestFor: "Moms, talkers, expressive faces.",
  },

  // ===== FAMILY SCRIPTS =====
  {
    id: "family_said_what",
    name: "The Family Said What?",
    bucket: "Family Scripts",
    topicWorld: "Family",
    description: "React to things family says that are funny, loaded, or too familiar.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 15,
    idea: "React to a thing family says — funny, loaded, or too familiar.",
    vibe: "Relatable, funny, a little triggering. Everyone's family says these.",
    whatToDrop: "Your reaction to the family line. 10-15 seconds.",
    easyWay: "Just react honestly.",
    examples: [
      "You got big.",
      "When are you having another baby?",
      "You look tired.",
      "You never come around.",
      "That's just how they are.",
      "Family is family.",
    ],
    makeItYours: "Your reaction, your face. The family line is the prompt — your reaction is the content.",
    seoPhrase: "things family says reaction video",
    captionStarter: "The family said what? What would your face do?",
    hashtagStarter: ["#AZOffScript", "#FamilySaidWhat", "#FamilyScripts", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "React to things family says that are funny, loaded, or too familiar.",
    whatEachPersonDoes: "Hear the family line, react.",
    whatToSend: "Your reaction. 10-15 seconds.",
    examplePrompts: [
      "You got big.",
      "When are you having another baby?",
      "You look tired.",
      "You never come around.",
      "That's just how they are.",
      "Family is family.",
    ],
    whyItWorks: "Everyone's family says these things. Instant relatability and sharing.",
    bestFor: "Talkers, expressive faces, group stitch.",
  },
  {
    id: "be_the_bigger_person_court",
    name: "Be the Bigger Person Court",
    bucket: "Family Scripts",
    topicWorld: "Family",
    description: "The room decides if someone should be the bigger person or finally stop doing that.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 20,
    idea: "We give you a situation. You decide: be the bigger person or finally stop?",
    vibe: "Honest, a little heavy, very real. The social script being broken.",
    whatToDrop: "Your verdict on the situation. 15-20 seconds.",
    easyWay: "Just say what you'd do and why.",
    examples: [
      "They never apologize.",
      "They always make you look dramatic.",
      "They invite everybody except you.",
      "They only call when they need something.",
      "They say 'that's just how I am.'",
    ],
    makeItYours: "Your verdict, your reasoning. This is literally a social script being broken.",
    seoPhrase: "be the bigger person or stop reaction",
    captionStarter: "Be the bigger person... or finally stop? You decide.",
    hashtagStarter: ["#AZOffScript", "#BiggerPersonCourt", "#FamilyScripts", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "The room decides if someone should be the bigger person or finally stop doing that.",
    whatEachPersonDoes: "Hear the situation, give your verdict.",
    whatToSend: "Your verdict + quick reason. 15-20 seconds.",
    examplePrompts: [
      "They never apologize.",
      "They always make you look dramatic.",
      "They invite everybody except you.",
      "They only call when they need something.",
      "They say 'that's just how I am.'",
    ],
    whyItWorks: "Very AZ Off Script — literally a social script being broken. Deep relatability.",
    bestFor: "Talkers, group stitch, solo home clips.",
  },

  // ===== ADULTING & ERRANDS =====
  {
    id: "can_this_be_an_email",
    name: "Can This Be an Email?",
    bucket: "Adulting & Errands",
    topicWorld: "Adulting",
    description: "The room reacts to things that should not require a meeting, phone call, or big explanation.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 15,
    idea: "React to something that should have been an email, not a whole thing.",
    vibe: "Exasperated, funny, relatable. The 'why is this a meeting' energy.",
    whatToDrop: "Your reaction to the thing. 10-15 seconds.",
    easyWay: "Just react and say why it should have been an email.",
    examples: [
      "A 45-minute meeting for one question.",
      "A phone call that could have been a text.",
      "A group chat with 89 unread messages.",
      "A 'quick favor' that takes three hours.",
      "A school email with 12 attachments.",
    ],
    makeItYours: "Your reaction, your exasperation. The 'why is this a thing' energy is the content.",
    seoPhrase: "can this be an email reaction video",
    captionStarter: "Can this be an email? Why is this a whole thing?",
    hashtagStarter: ["#AZOffScript", "#CanThisBeAnEmail", "#AdultingErrands", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "The room reacts to things that should not require a meeting, phone call, or big explanation.",
    whatEachPersonDoes: "React to the thing and say why it should have been simpler.",
    whatToSend: "Your reaction. 10-15 seconds.",
    examplePrompts: [
      "A 45-minute meeting for one question.",
      "A phone call that could have been a text.",
      "A group chat with 89 unread messages.",
      "A 'quick favor' that takes three hours.",
      "A school email with 12 attachments.",
    ],
    whyItWorks: "Everyone who works or parents has lived this. Instant relatability.",
    bestFor: "Talkers, expressive faces, solo home clips.",
  },
  {
    id: "errand_olympics",
    name: "Errand Olympics",
    bucket: "Adulting & Errands",
    topicWorld: "Adulting",
    description: "Rank everyday errands by how annoying they are.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 20,
    idea: "We give you a list of errands. You rank them from worst to least worst.",
    vibe: "Quick, opinionated, funny. The errand dread is the content.",
    whatToDrop: "Your ranking of the errands. 15-20 seconds.",
    easyWay: "Just hold up fingers or say the number for each errand.",
    examples: [
      "Grocery pickup.",
      "DMV.",
      "School pickup.",
      "Returning Amazon packages.",
      "Taking kids to appointments.",
      "Going to one store and somehow needing four.",
    ],
    makeItYours: "Your ranking, your logic. The errand dread is universal.",
    seoPhrase: "errand olympics ranking reaction",
    captionStarter: "We ranked these errands by dread. Do you agree?",
    hashtagStarter: ["#AZOffScript", "#ErrandOlympics", "#AdultingErrands", "#ArizonaCreators", "#RankIt"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "Rank everyday errands by how annoying they are.",
    whatEachPersonDoes: "Rank the errands from worst to least worst.",
    whatToSend: "Your ranking. 15-20 seconds. Hold up fingers or say numbers.",
    examplePrompts: [
      "Grocery pickup.",
      "DMV.",
      "School pickup.",
      "Returning Amazon packages.",
      "Taking kids to appointments.",
      "Going to one store and somehow needing four.",
    ],
    whyItWorks: "Everyone hates errands. The ranking debate is instant engagement.",
    bestFor: "Talkers, group stitch, solo home clips.",
  },
  {
    id: "nervous_system_said_no",
    name: "My Nervous System Said No",
    bucket: "Adulting & Errands",
    topicWorld: "Adulting",
    description: "Quick reactions to overstimulating everyday things.",
    effort: "2-Min Drop",
    timeEstimate: "1-2 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: false,
    maxSeconds: 10,
    idea: "React to something overstimulating — your nervous system said no.",
    vibe: "Quick, honest, a little overwhelmed. The 'I can't today' energy.",
    whatToDrop: "Your reaction to the overstimulating thing. 5-10 seconds.",
    easyWay: "Just react with your face or one line.",
    examples: [
      "Too many people talking at once.",
      "A surprise phone call.",
      "A loud store.",
      "A last-minute plan.",
      "A text that says 'call me.'",
    ],
    makeItYours: "Your reaction, your face. The overwhelm is the content.",
    seoPhrase: "nervous system said no reaction",
    captionStarter: "My nervous system said no. What's yours?",
    hashtagStarter: ["#AZOffScript", "#NervousSystemSaidNo", "#AdultingErrands", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "Quick reactions to overstimulating everyday things.",
    whatEachPersonDoes: "React to the overstimulating thing with your face or one line.",
    whatToSend: "Your reaction. 5-10 seconds.",
    examplePrompts: [
      "Too many people talking at once.",
      "A surprise phone call.",
      "A loud store.",
      "A last-minute plan.",
      "A text that says 'call me.'",
    ],
    whyItWorks: "Overstimulation is a universal feeling. Quiet and loud personalities both relate.",
    bestFor: "Talkers, quiet faces, solo home clips.",
  },

  // ===== BEAUTY / STYLE / PRESENTATION =====
  {
    id: "outfit_entered_chat",
    name: "The Outfit Has Entered the Chat",
    bucket: "Beauty / Style / Presentation",
    topicWorld: "Beauty / Style",
    description: "Outfit/style reaction without turning the page into fashion only.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 15,
    idea: "Show your fit or react to an outfit prompt. Keep it real, not fashion-only.",
    vibe: "Cool, fun, confident. The outfit is the personality.",
    whatToDrop: "Your fit check or outfit reaction. 10-15 seconds.",
    easyWay: "Just show your outfit or react to the prompt.",
    examples: [
      "Comfortable but still cute.",
      "Overdressed or everybody else underdressed?",
      "The outfit I planned vs the outfit I survived in.",
      "Cute but can I chase a kid in it?",
      "This look says I tried, but not too hard.",
    ],
    makeItYours: "Your style, your fit. The outfit is the personality.",
    seoPhrase: "outfit entered the chat fit check",
    captionStarter: "The outfit has entered the chat. How would you style it?",
    hashtagStarter: ["#AZOffScript", "#OutfitEnteredChat", "#OffScriptLooks", "#ArizonaCreators", "#FitCheck"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "Outfit/style reaction without turning the page into fashion only.",
    whatEachPersonDoes: "Show your fit or react to the outfit prompt.",
    whatToSend: "Your fit check or outfit reaction. 10-15 seconds.",
    examplePrompts: [
      "Comfortable but still cute.",
      "Overdressed or everybody else underdressed?",
      "The outfit I planned vs the outfit I survived in.",
      "Cute but can I chase a kid in it?",
      "This look says I tried, but not too hard.",
    ],
    whyItWorks: "Outfit content is high-engagement. Keeps the brand styled without being fashion-only.",
    bestFor: "Talkers, quiet faces, group day, solo home clips.",
  },
  {
    id: "bad_angle_veto",
    name: "The Bad Angle Veto",
    bucket: "Beauty / Style / Presentation",
    topicWorld: "Beauty / Style",
    description: "Funny but useful rule: not every clip gets posted if it does someone dirty.",
    effort: "2-Min Drop",
    timeEstimate: "1-2 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 10,
    idea: "React to an angle or clip — would you approve it, crop it, or delete it?",
    vibe: "Funny, honest, a little vain. The 'I know my angles' energy.",
    whatToDrop: "Your verdict on the angle/clip. 5-10 seconds.",
    easyWay: "Just say post it, crop it, or delete it.",
    examples: [
      "Would you approve this angle?",
      "Post it, crop it, or delete it?",
      "Is this funny or is this betrayal?",
      "Would you send this to the group chat first?",
    ],
    makeItYours: "Your verdict, your honesty. The vanity is the comedy.",
    seoPhrase: "bad angle veto post crop delete",
    captionStarter: "Post it, crop it, or delete it? Be honest.",
    hashtagStarter: ["#AZOffScript", "#BadAngleVeto", "#OffScriptLooks", "#ArizonaCreators", "#FitCheck"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "Funny but useful rule: not every clip gets posted if it does someone dirty.",
    whatEachPersonDoes: "React to an angle or clip and give your verdict.",
    whatToSend: "Your verdict. 5-10 seconds.",
    examplePrompts: [
      "Would you approve this angle?",
      "Post it, crop it, or delete it?",
      "Is this funny or is this betrayal?",
      "Would you send this to the group chat first?",
    ],
    whyItWorks: "Everyone has a bad angle story. Funny and relatable.",
    bestFor: "Talkers, expressive faces, group stitch.",
  },

  // ===== SOFT TRUTHS =====
  {
    id: "the_polite_line",
    name: "The Polite Line",
    bucket: "Soft Truths",
    topicWorld: "Soft Truths",
    description: "What you say politely vs what your face said.",
    effort: "5-Min Drop",
    timeEstimate: "2-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 15,
    idea: "We give you a polite phrase. You say it — then show what your face actually said.",
    vibe: "Funny, honest, relatable. The polite mask is the comedy.",
    whatToDrop: "Your polite phrase + your real face. 10-15 seconds.",
    easyWay: "Say the polite thing, then drop the mask.",
    examples: [
      "I'll think about it.",
      "No worries.",
      "That makes sense.",
      "I'm good.",
      "Let me check my schedule.",
    ],
    makeItYours: "Your polite face, your real face. The contrast is the content.",
    seoPhrase: "the polite line what you say vs what you mean",
    captionStarter: "What you said politely vs what your face said. Same?",
    hashtagStarter: ["#AZOffScript", "#ThePoliteLine", "#SoftTruths", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "What you say politely vs what your face said.",
    whatEachPersonDoes: "Say the polite phrase, then show your real face.",
    whatToSend: "Your polite phrase + your real face. 10-15 seconds.",
    examplePrompts: [
      "I'll think about it.",
      "No worries.",
      "That makes sense.",
      "I'm good.",
      "Let me check my schedule.",
    ],
    whyItWorks: "Everyone wears the polite mask. The reveal is the comedy.",
    bestFor: "Talkers, expressive faces, quiet faces.",
  },

  // ===== PUBLIC BEHAVIOR =====
  {
    id: "act_normal_challenge",
    name: "Act Normal Challenge",
    bucket: "Public Behavior",
    topicWorld: "Public Behavior",
    description: "The room reacts to situations where people expect you to act normal but the face gives it away.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 15,
    idea: "React to a situation where you're supposed to act normal — but your face gives it away.",
    vibe: "Funny, relatable, the 'I tried to keep it together' energy.",
    whatToDrop: "Your reaction to the situation. 10-15 seconds.",
    easyWay: "Just react with your face or one line.",
    examples: [
      "Someone says something wild in public.",
      "The total at checkout is higher than expected.",
      "You see someone you were avoiding.",
      "Your kid repeats something private.",
      "Someone asks a question they already know the answer to.",
    ],
    makeItYours: "Your face, your reaction. The 'trying to act normal' is the content.",
    seoPhrase: "act normal challenge reaction video",
    captionStarter: "Act normal. Could you keep your face straight?",
    hashtagStarter: ["#AZOffScript", "#ActNormalChallenge", "#PublicBehavior", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "The room reacts to situations where people expect you to act normal but the face gives it away.",
    whatEachPersonDoes: "React to the situation with your face or one line.",
    whatToSend: "Your reaction. 10-15 seconds.",
    examplePrompts: [
      "Someone says something wild in public.",
      "The total at checkout is higher than expected.",
      "You see someone you were avoiding.",
      "Your kid repeats something private.",
      "Someone asks a question they already know the answer to.",
    ],
    whyItWorks: "Everyone has tried to keep a straight face in public. Instant relatability.",
    bestFor: "Talkers, expressive faces, quiet faces, group stitch.",
  },
  {
    id: "dont_make_it_awkward",
    name: "Don't Make It Awkward",
    bucket: "Public Behavior",
    topicWorld: "Public Behavior",
    description: "The room decides if someone should say the truth or keep the peace.",
    effort: "5-Min Drop",
    timeEstimate: "3-5 min",
    homeFriendly: true,
    needsTalking: true,
    needsEditing: false,
    adminStitches: true,
    maxSeconds: 15,
    idea: "We give you a situation. You decide: say the truth or keep the peace?",
    vibe: "Honest, funny, a little uncomfortable. The 'do I say it' debate.",
    whatToDrop: "Your verdict on the situation. 10-15 seconds.",
    easyWay: "Just say what you'd do and why.",
    examples: [
      "They have food in their teeth.",
      "Their man is embarrassing them.",
      "The group plan is too expensive.",
      "Nobody likes the restaurant choice.",
      "Someone's story does not add up.",
    ],
    makeItYours: "Your verdict, your reasoning. The 'do I say it' debate is the content.",
    seoPhrase: "don't make it awkward say truth or keep peace",
    captionStarter: "Say the truth or keep the peace? What would you do?",
    hashtagStarter: ["#AZOffScript", "#DontMakeItAwkward", "#PublicBehavior", "#ArizonaCreators", "#ReactionVideo"],
    platforms: ["TikTok", "Reels", "Shorts", "Facebook"],
    whatThisIs: "The room decides if someone should say the truth or keep the peace.",
    whatEachPersonDoes: "Hear the situation, give your verdict.",
    whatToSend: "Your verdict + quick reason. 10-15 seconds.",
    examplePrompts: [
      "They have food in their teeth.",
      "Their man is embarrassing them.",
      "The group plan is too expensive.",
      "Nobody likes the restaurant choice.",
      "Someone's story does not add up.",
    ],
    whyItWorks: "Everyone has been in this situation. The debate is instant.",
    bestFor: "Talkers, group stitch, solo home clips.",
  },
];

export function getTemplate(id: string): QuickDropTemplate | undefined {
  return QUICK_DROP_TEMPLATES.find((t) => t.id === id);
}

export function getTemplatesByBucket(bucket: string): QuickDropTemplate[] {
  return QUICK_DROP_TEMPLATES.filter((t) => t.bucket === bucket);
}

export function getExampleFor(template: QuickDropTemplate, memberName?: string): string | null {
  if (!template.examples || template.examples.length === 0) return null;
  if (!memberName) return template.examples[0];
  const firstName = memberName.split(" ")[0];
  const match = template.examples.find((line) =>
    line.toLowerCase().includes(firstName.toLowerCase())
  );
  return match ?? template.examples[0];
}
